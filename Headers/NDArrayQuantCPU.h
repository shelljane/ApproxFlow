#ifndef __APPROXFLOW_NDARRAYQUANTCPU__
#define __APPROXFLOW_NDARRAYQUANTCPU__
...
public:
    ...
    NDArrayQuantCPU(FloatType S, AddType Z, const std::vector<size_t> &shape);
    NDArrayQuantCPU(FloatType S, AddType Z, const std::vector<size_t> &shape, const std::vector<Scalar> &arr);
    
    NDArrayQuantCPU(const NDArrayQuantCPU<Scalar> &ndarray); 
    NDArrayQuantCPU(NDArrayQuantCPU<Scalar> &&ndarray); 
    
    const std::vector<size_t> &shape() const {return _shape; }
    size_t size() const {return _size; }
    size_t length() const {return _size; }
    
    const NDArrayQuantCPU &resize(const std::vector<size_t> &shape); 
    NDArrayQuantCPU reshape(const std::vector<size_t> &shape) const; 
    ...
    NDArrayQuantCPU get(const std::vector<Range> &index) const; 
    void set(const std::vector<Range> &index, const NDArrayQuantCPU &value); 
    ...
    const NDArrayQuantCPU &operator = (const NDArrayQuantCPU &ndarray); 
    const NDArrayQuantCPU &operator = (NDArrayQuantCPU &&ndarray); 
    
    static NDArrayQuantCPU add(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2); 
    static NDArrayQuantCPU subtract(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2); 
    static NDArrayQuantCPU multiply(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2); 
    static NDArrayQuantCPU matMul(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2); 
    static NDArrayQuantCPU matMAD(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2, const std::vector<AccumType> &bias); 
    static NDArrayQuantCPU matMADReLU(FloatType SReLU, AddType ZReLU, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2, const std::vector<AccumType> &bias); 
    
    NDArrayQuantCPU T() const; 
    NDArrayQuantCPU T(const std::vector<size_t> &axis) const; 
    
    NDArrayQuantCPU ReLU() const; 
//     NDArrayQuantCPU mask01() const; 
    
    std::vector<size_t> argmax() const; 
    size_t posmax() const; 
//     AccumType sum() const; 
//     NDArrayQuantCPU sum(size_t axis = 0) const; 
    
    NDArrayQuantCPU maxPool(const std::vector<size_t> &size, const std::vector<size_t> &stride, bool padding = true) const; 
    NDArrayQuantCPU im2col(const std::vector<size_t> &size, const std::vector<size_t> &stride, bool padding = true) const; 
//     NDArrayQuantCPU col2im(const std::vector<size_t> &origin, const std::vector<size_t> &size, const std::vector<size_t> &stride, bool padding = true) const; 
    ...
    NDArrayQuantCPU addChWise(FloatType S, AddType Z, const NDArrayQuantCPU &ndarray) const; 
//     NDArrayQuantCPU addChWiseRev(const NDArrayQuantCPU &ndarray) const; 
    
    const NDArrayQuantCPU &operator += (const Scalar &value); 
    const NDArrayQuantCPU &operator -= (const Scalar &value); 
    const NDArrayQuantCPU &operator &= (const Scalar &value); 
    const NDArrayQuantCPU &operator *= (const Scalar &value); 
    const NDArrayQuantCPU &operator += (const NDArrayQuantCPU &ndarray); 
    ...
    NDArrayQuantCPU operator + () const; 
    NDArrayQuantCPU operator - () const; 
    NDArrayQuantCPU operator + (const Scalar &value) const; 
    NDArrayQuantCPU operator - (const Scalar &value) const; 
    NDArrayQuantCPU operator & (const Scalar &value) const; 
    NDArrayQuantCPU operator * (const Scalar &value) const; 
    NDArrayQuantCPU operator + (const NDArrayQuantCPU &ndarray) const; 
    NDArrayQuantCPU operator - (const NDArrayQuantCPU &ndarray) const; 
    NDArrayQuantCPU operator & (const NDArrayQuantCPU &ndarray) const; 
}; 

template<typename Scalar>
bool NDArrayQuantCPU<Scalar>::_checkRange(const std::vector<size_t> &index) const
{
...
template<typename Scalar>
bool NDArrayQuantCPU<Scalar>::_checkRange(const std::vector<Range> &index) const
{
    if(index.size() != _shape.size())
    {
...
template<typename Scalar>
size_t NDArrayQuantCPU<Scalar>::_getIndex(const std::vector<size_t> &index) const
{
...
template<typename Scalar>
size_t NDArrayQuantCPU<Scalar>::_mulUp(const std::vector<size_t> &shape)
{
    size_t size = 1; 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar>::NDArrayQuantCPU(): _S(), _Z(), _size(0), _shape(), _array() {}

template<typename Scalar>
NDArrayQuantCPU<Scalar>::NDArrayQuantCPU(FloatType S, AddType Z, const std::vector<size_t> &shape): _S(S), _Z(Z), _size(1), _shape(shape), _array() 
{
...
template<typename Scalar>
NDArrayQuantCPU<Scalar>::NDArrayQuantCPU(FloatType S, AddType Z, const std::vector<size_t> &shape, const std::vector<Scalar> &arr): _S(S), _Z(Z), _size(1), _shape(shape), _array(arr) 
{
    for(size_t idx = 0; idx < _shape.size(); idx++)
    {
        _size *= _shape[idx]; 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar>::NDArrayQuantCPU(const NDArrayQuantCPU<Scalar> &ndarray): _S(ndarray._S), _Z(ndarray._Z), _size(ndarray._size), _shape(ndarray._shape), _array(ndarray._array) {}

template<typename Scalar>
NDArrayQuantCPU<Scalar>::NDArrayQuantCPU(NDArrayQuantCPU<Scalar> &&ndarray): _S(ndarray._S), _Z(ndarray._Z), _size(ndarray._size), _shape(ndarray._shape), _array(std::move(ndarray._array)) {}


template<typename Scalar>
const NDArrayQuantCPU<Scalar> &NDArrayQuantCPU<Scalar>::resize(const std::vector<size_t> &shape)
{
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::reshape(const std::vector<size_t> &shape) const
{
    assert(_mulUp(_shape) == _mulUp(shape)); 
    
    NDArrayQuantCPU<Scalar> result = *this; 
    result.resize(shape);
...
template<typename Scalar>
Scalar &NDArrayQuantCPU<Scalar>::operator [] (size_t index)
{
    return _array[index]; 
}

template<typename Scalar>
const Scalar &NDArrayQuantCPU<Scalar>::operator [] (size_t index) const
{
    return _array[index]; 
}

template<typename Scalar>
Scalar &NDArrayQuantCPU<Scalar>::operator [] (const std::vector<size_t> &index)
{
    return _array[_getIndex(index)]; 
}

template<typename Scalar>
const Scalar &NDArrayQuantCPU<Scalar>::operator [] (const std::vector<size_t> &index) const
{
    return _array[_getIndex(index)]; 
}

template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::get(const std::vector<Range> &index) const
{
    assert(_checkRange(index)); 
    ...
    NDArrayQuantCPU<Scalar> result(_S, _Z, shape); 
...
template<typename Scalar>
void NDArrayQuantCPU<Scalar>::set(const std::vector<Range> &index, const NDArrayQuantCPU<Scalar> &value)
{
    assert(_checkRange(index)); 
...
template<typename Scalar>
void NDArrayQuantCPU<Scalar>::print() const
{
    std::vector<size_t> current(_shape.size(), 0); 
    std::cout << std::endl << "NDArrayQuantCPU: S = " << _S << "; Z = " << _Z << std::endl; 
...
template<typename Scalar>
typename NDArrayQuantCPU<Scalar>::NDArrayFloat NDArrayQuantCPU<Scalar>::eval() const
{
...
template<typename Scalar>
const NDArrayQuantCPU<Scalar> &NDArrayQuantCPU<Scalar>::operator = (const NDArrayQuantCPU<Scalar> &ndarray)
{
    _S = ndarray._S;
    _Z = ndarray._Z; 
    _size = ndarray._size; 
...
template<typename Scalar>
const NDArrayQuantCPU<Scalar> &NDArrayQuantCPU<Scalar>::operator = (NDArrayQuantCPU &&ndarray)
{
    _S = ndarray._S;
    _Z = ndarray._Z; 
    _size = ndarray._size; 
...
template<typename Scalar>
template<typename FixedNum>
FixedNum NDArrayQuantCPU<Scalar>::_toFixedNum(float r)
{
    assert(r >= 0.0 && r <= 1.0); 
//     if(!(r >= 0.0 && r <= 1.0))
//     {
...
template<typename Scalar>
template<typename FixedNum, typename RaisedNum>
FixedNum NDArrayQuantCPU<Scalar>::_truncate(RaisedNum a)
{
...
template<typename Scalar>
template<typename FixedNum, typename RaisedNum>
FixedNum NDArrayQuantCPU<Scalar>::_addFixedNum(FixedNum a, FixedNum b)
{
...
template<typename Scalar>
template<typename FixedNum, typename RaisedNum>
FixedNum NDArrayQuantCPU<Scalar>::_subtractFixedNum(FixedNum a, FixedNum b)
{
    RaisedNum raised = static_cast<RaisedNum>(a) - static_cast<RaisedNum>(b); 
...
template<typename Scalar>
template<typename FixedNum, typename RaisedNum>
FixedNum NDArrayQuantCPU<Scalar>::_mulFixedNum(FixedNum a, FixedNum b)
{
    RaisedNum raised; 
...
template<typename Scalar>
template<typename FixedNum, typename FixedNumUnsigned>
FixedNum NDArrayQuantCPU<Scalar>::_mulSignedFixedNum(FixedNum signedNum, FixedNumUnsigned unsignedNum)
{
    long tmp; 
    unsigned long tmp2; 
    if(signedNum < 0)
...
template<typename Scalar>
template<typename FixedNum, typename FixedNumUnsigned>
FixedNum NDArrayQuantCPU<Scalar>::_mulSignedFixedNumShift(FixedNum signedNum, FixedNumUnsigned unsignedNum, unsigned shift)
{
    long tmp; 
    unsigned long tmp2; 
    if(signedNum < 0)
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::add(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2)
{
    assert(arr1._size == arr2._size); 
    assert(S >= arr1._S && S >= arr2._S); 
    
    NDArrayQuantCPU<Scalar> result(S, Z, arr1._shape); 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::subtract(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2)
{
    assert(arr1._size == arr2._size); 
    assert(S >= arr1._S && S >= arr2._S); 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::multiply(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2)
{
    assert(arr1._size == arr2._size); 
    assert(S >= arr1._S && S >= arr2._S); 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::matMul(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2)
{
...
//     std::cout << "scale_f: " << scale_f << std::endl; 
    ...
    NDArrayQuantCPU<Scalar> tmp = arr2.T(); 
    NDArrayQuantCPU<Scalar> result(S, Z, {arr1._shape[0], tmp._shape[0]}); 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::matMAD(FloatType S, AddType Z, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2, const std::vector<AccumType> &bias)
{
    assert(arr1._shape.size() == 2 && arr2._shape.size() == 2);
    assert(arr1._shape[1] == arr2._shape[0]);
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::matMADReLU(FloatType SReLU, AddType ZReLU, const NDArrayQuantCPU &arr1, const NDArrayQuantCPU &arr2, const std::vector<AccumType> &bias)
{
    assert(arr1._shape.size() == 2 && arr2._shape.size() == 2);
...
//     std::cout << "scale_f: " << scale_f << std::endl; 
    ...
    NDArrayQuantCPU<Scalar> tmp = arr2.T(); 
    NDArrayQuantCPU<Scalar> result(SReLU, ZReLU, {arr1._shape[0], tmp._shape[0]}); 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::T() const
{
    ...
    NDArrayQuantCPU<Scalar> result(_S, _Z, {_shape[1], _shape[0]}); 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::T(const std::vector<size_t> &axis) const
{
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::ReLU() const
{
    NDArrayQuantCPU<Scalar> result(_S, _Z, _shape); 
    for(size_t idx = 0; idx < _size; idx++)
    {
...
template<typename Scalar>
std::vector<size_t> NDArrayQuantCPU<Scalar>::argmax() const
{
...
template<typename Scalar>
size_t NDArrayQuantCPU<Scalar>::posmax() const
{
    size_t tmpPos = 0; 
    Scalar tmpMax = _array[0]; 
    for(size_t idx = 0; idx < _size; idx++)
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::addChWise(FloatType S, AddType Z, const NDArrayQuantCPU &ndarray) const
{
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::maxPool(const std::vector<size_t> &size, const std::vector<size_t> &stride, bool padding) const 
{
    assert(_shape.size() == 3); 
    assert(size.size() == 2); 
    ...
    NDArrayQuantCPU<Scalar> result(_S, _Z, {shape[0], shape[1], _shape[2]}); 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::im2col(const std::vector<size_t> &size, const std::vector<size_t> &stride, bool padding) const
{
    assert(_shape.size() == 3); 
    assert(size.size() == 2); 
    assert(stride.size() == 2); 
    ...
    NDArrayQuantCPU<Scalar> result(_S, _Z, {shape[0] * shape[1], size[0] * size[1] * _shape[2]}); 
...
template<typename Scalar>
std::vector<size_t> NDArrayQuantCPU<Scalar>::sizeIm2col(const std::vector<size_t> &size, const std::vector<size_t> &stride, bool padding) const
{
    assert(_shape.size() == 3); 
    assert(size.size() == 2); 
    assert(stride.size() == 2); 
...
template<typename Scalar>
std::vector<size_t> NDArrayQuantCPU<Scalar>::sizeIm2Col_ImOnly(const std::vector<size_t> &size, const std::vector<size_t> &stride, bool padding) const
{
    assert(_shape.size() == 3); 
    assert(size.size() == 2); 
    assert(stride.size() == 2); 
...
template<typename Scalar>
std::vector<size_t> NDArrayQuantCPU<Scalar>::sizePool(const std::vector<size_t> &size, const std::vector<size_t> &stride, bool padding) const
{
    assert(_shape.size() == 3); 
    assert(size.size() == 2); 
    assert(stride.size() == 2); 
...
template<typename Scalar>
const NDArrayQuantCPU<Scalar> &NDArrayQuantCPU<Scalar>::operator += (const Scalar &value)
{
    for(size_t idx = 0; idx < _size; idx++)
    {
        _array[idx] += value; 
...
template<typename Scalar>
const NDArrayQuantCPU<Scalar> &NDArrayQuantCPU<Scalar>::operator += (const NDArrayQuantCPU &ndarray)
{
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::operator + () const {return *this; }

template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::operator - () const 
{
    NDArrayQuantCPU<Scalar> result(_S, ~_Z, _shape); 
    for(size_t idx = 0; idx < _size; idx++)
    {
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::operator + (const Scalar &value) const
{
    NDArrayQuantCPU<Scalar> result = *this; 
    result += value; 
...
template<typename Scalar>
NDArrayQuantCPU<Scalar> NDArrayQuantCPU<Scalar>::operator + (const NDArrayQuantCPU &ndarray) const
{
    NDArrayQuantCPU<Scalar> result = *this; 
    result += ndarray; 
